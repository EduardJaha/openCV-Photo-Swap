...
void Swapper::oddC_evenR(const cv::Mat &image) {
  ...
  cv::Rect rectangleTopLeft_resImg(0, 0, (size_of_res.width / 2),
                                   size_of_res.height / 2);
  cv::Rect rectangleTopRight_resImg(size_of_res.width / 2, 0,
                                    (size_of_res.width / 2) + 1,
                                    size_of_res.height / 2);
  cv::Rect rectangleBottomLeft_resImg(0, (size_of_res.height / 2),
                                      (size_of_res.width / 2),
                                      (size_of_res.height / 2));
  cv::Rect rectangleBottomRight_resImg(
      size_of_res.width / 2, size_of_res.height / 2,
